...
def Equal_sharing(num_of_vehicles, timesteps,
    ...
    for t in range(int(arrival_time[0])+1, timesteps-5):
        ...
        final_energy_needed = np.copy(terminal_states[:vehicle_ending_index])
        ...
        '''
        updated_val = np.minimum(u_val, np.ones_like(u_val) * max_power)
    ...
    #plt.plot(all_state_EDF[0,:-5]/B,label='SoC')
    #plt.plot(u_mat[0,:-5],label='charging_power')
    #plt.show()
